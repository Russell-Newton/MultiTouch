...
TEST_F(TestPreprocessing, TestStationaryDownUp) {
    touch_event_t down = {TOUCH_EVENT_DOWN, 0, 0, 0, TOUCH_ID_UNDEFINED, 0};
    touch_event_t up   = {TOUCH_EVENT_UP, 0, 0, 0, TOUCH_ID_UNDEFINED, 0};
    for (int i = 0; i < 5; i++) {
        down.t = i * 2;
        up.t   = i * 2 + 1;
...
TEST_F(TestPreprocessing, TestStationaryDownMultiple) {
    for (int i = 0; i < MAX_TOUCHES; i++) {
        touch_event_t newTouch = {TOUCH_EVENT_DOWN, (float)i, 0, 0, TOUCH_ID_UNDEFINED, 0};
        process_touch_event(&newTouch, nullptr, 0);
        EXPECT_EQ(newTouch.group, i);
    }
    touch_event_t newTouch = {TOUCH_EVENT_DOWN, (float)MAX_TOUCHES, 0, (float)MAX_TOUCHES, TOUCH_ID_UNDEFINED, 0};
    process_touch_event(&newTouch, nullptr, 0);
...
TEST_F(TestPreprocessing, TestMoveUpUndefined) {
    touch_event_t badEvent = {TOUCH_EVENT_MOVE, 0, 0, 0, TOUCH_ID_UNDEFINED, 0};
...
TEST_F(TestPreprocessing, TestDownUpMultiple) {
    ...
    for (int i = 0; i < MAX_TOUCHES; i++) {
        downTouches[i] = {TOUCH_EVENT_DOWN, (float)i, 0, 0, TOUCH_ID_UNDEFINED, 0};
        upTouches[i]   = {TOUCH_EVENT_UP, (float)i, 0, 0, TOUCH_ID_UNDEFINED, 0};
...
TEST_F(TestPreprocessing, TestSwipeSingle) {
    touch_event_t downEvent = {TOUCH_EVENT_DOWN, 0, 0, 0, TOUCH_ID_UNDEFINED, 0};
    process_touch_event(&downEvent, nullptr, 0);
    EXPECT_EQ(downEvent.group, 0);
    for (int i = 0; i < 10; i++) {
        touch_event_t moveEvent = {TOUCH_EVENT_MOVE, i * 0.1f, 0, 0, TOUCH_ID_UNDEFINED, 0};
        process_touch_event(&moveEvent, nullptr, 0);
        EXPECT_EQ(moveEvent.group, 0);
    }
    touch_event_t upEvent = {TOUCH_EVENT_UP, 0, 0, 0, TOUCH_ID_UNDEFINED, 0};
...
TEST_F(TestPreprocessing, TestSwipeMultiple) {
    ...
    for (int touch = 0; touch < MAX_TOUCHES; touch++) {
        touch_event_t downEvent = {TOUCH_EVENT_DOWN, 0, (float)touch, 0, TOUCH_ID_UNDEFINED, 0};
        process_touch_event(&downEvent, nullptr, 0);
        EXPECT_EQ(downEvent.group, touch);
    }
    for (int i = 0; i < steps; i++) {
        for (int touch = 0; touch < MAX_TOUCHES; touch++) {
            touch_event_t moveEvent = {TOUCH_EVENT_MOVE, i * 0.1f, (float)touch, 0, TOUCH_ID_UNDEFINED, 0};
            process_touch_event(&moveEvent, nullptr, 0);
            EXPECT_EQ(moveEvent.group, touch);
        }
    }
    for (int touch = 0; touch < MAX_TOUCHES; touch++) {
        touch_event_t upEvent = {TOUCH_EVENT_UP, steps * 0.1f, (float)touch, 0, TOUCH_ID_UNDEFINED, 0};
        process_touch_event(&upEvent, nullptr, 0);
...
TEST_F(TestPreprocessing, TestSwipeMultipleWithUp) {
    ...
    for (int touch = 0; touch < MAX_TOUCHES; touch++) {
        touch_event_t event = touch_event_t({TOUCH_EVENT_DOWN, 0, (float)touch, 0, TOUCH_ID_UNDEFINED, 0});
    ...
    for (int i = 0; i < steps; i++) {
        for (int touch = 0; touch < MAX_TOUCHES; touch++) {
            if (i == ups[touch]) {
                touch_event_t upEvent = {TOUCH_EVENT_UP, i * 0.1f, (float)touch, 0, TOUCH_ID_UNDEFINED, 0};
                process_touch_event(&upEvent, nullptr, 0);
                EXPECT_EQ(upEvent.group, touch);
                touch_event_t downEvent = {TOUCH_EVENT_DOWN, i * 0.1f, (float)touch, 0, TOUCH_ID_UNDEFINED, 0};
                process_touch_event(&downEvent, nullptr, 0);
                EXPECT_EQ(downEvent.group, touch);
            } else {
                touch_event_t event = {TOUCH_EVENT_MOVE, i * 0.1f, (float)touch, 0, TOUCH_ID_UNDEFINED, 0};
                process_touch_event(&event, nullptr, 0);
                EXPECT_EQ(event.group, touch);
            }
        }
    ...
    for (int touch = 0; touch < MAX_TOUCHES; touch++) {
        touch_event_t event = {TOUCH_EVENT_UP, steps * 0.1f, (float)touch, 0, TOUCH_ID_UNDEFINED, 0};
        process_touch_event(&event, nullptr, 0);
        EXPECT_EQ(event.group, touch);
    }
}
